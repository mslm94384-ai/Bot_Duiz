...
const run = async (m, { conn, bot, command }) => {
    ...
    if (!mime) return m.reply('⚔️ *رد على الفويس يا جندي عشان تغير صوته!* 🔥');
    await m.react("⏳")
    const media = await q.download();
    const link = await uploadTmpfiles(media);
    const { data: { seconds } } = await Utils.duration(link);
    
    if (seconds > 20) return m.reply("💀 *الصوت طويل يا جندي! مينفعش فوق 20 ثانية*");
    ...
    switch (command) {
        ...
        default: return m.reply("❌ *أمر غير معروف يا جندي!* 💢");
    }
